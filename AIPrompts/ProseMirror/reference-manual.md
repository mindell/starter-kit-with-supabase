...
Returns a resolved position if this is a cursor selection , and null otherwise.
...
`new [SelectionRange]: [ResolvedPos], [$to]: [ResolvedPos])`
...
Determines the distance  between the cursor and the end of the visible viewport at which point, when scrolling the cursor into view, scrolling takes place. Defaults to 0.
...
Test whether replacing the range between `from` and `to`  with the given replacement fragment  would leave the node's content valid. You can optionally pass `start` and `end` indices into the replacement fragment.
...
`static [fromArray]: readonly [Node][]) → [Fragment]`
...
Defines the default way a node of this type should be serialized to DOM/HTML ). Should return a DOM node or an [array structure] that describes one, with an optional number zero  in it to indicate where the node's content should be inserted.
...
`[lift]: [NodeRange], [target]: [number]) → [Transform]`
...
`[removeNodeMark]: [number], [mark]: [Mark] | [MarkType]) → [Transform]`
...
'Fit' a slice into a given position in the document, producing a [step] that inserts it. Will return null if there's no meaningful way to insert the slice here, or inserting it would be a no-op .
...
If a block node is selected, create an empty paragraph before  or after it.
...
Controls whether, when part of the selected range has the mark already and part doesn't, the mark is removed  or added .
...
[isJoinable]: fn: [Node], [after]: [Node]) → [boolean] |
...
A basic keymap containing bindings not specific to any schema. Binds the following keys ):
...
An implementation of an undo/redo history for ProseMirror. This history is *selective*, meaning it does not just roll back to a previous state but can undo some changes while keeping other, later changes intact. 

`[history]⁠?: Object = {}) → [Plugin]`
...
The delay between changes after which a new group should be started. Defaults to 500 . Note that when changes aren't adjacent, a new group is always started.
...
`[undoDepth]: [EditorState]) → any`

The amount of undoable events available in a given state.

`[redoDepth]: [EditorState]) → any`

The amount of redoable events available in a given editor state.

`[closeHistory]: [Transaction]) → [Transaction]`

Set a flag on the given transaction that will prevent further steps from being appended to an existing history event .
...
When enabled , if the current selection is a [text selection], its sides are mapped with a negative bias for this transaction, so that content inserted at the cursor ends up after the cursor. Users usually prefer this, but it isn't done by default for reasons of backwards compatibility.
...
`[keymap]: [Object]<[Command]>) → [Plugin]`

Create a keymap plugin for the given set of bindings.

Bindings should map key names to [command]-style functions, which will be called with `` arguments, and should return true when they've handled the key. Note that the view argument isn't part of the command protocol, but can be used as an escape hatch if a binding needs to directly interact with the UI.

Key names may be strings like `"Shift-Ctrl-Enter"`---a key identifier prefixed with zero or more modifiers. Key identifiers are based on the strings that can appear in [`KeyEvent.key`]. Use lowercase letters to refer to letter keys . You may use `"Space"` as an alias for the `" "` name.

Modifiers can be given in any order. `Shift-` , `Alt-` , `Ctrl-`  and `Cmd-`  are recognized. For characters that are created by holding shift, the `Shift-` prefix is implied, and should not be added explicitly.

You can use `Mod-` as a shorthand for `Cmd-` on Mac and `Ctrl-` on other platforms.

You can add multiple keymap plugins to an editor. The order in which they appear determines their precedence .

`[keydownHandler]: [Object]<[Command]>) → fn: [EditorView], [event]: [KeyboardEvent]) → [boolean]`

Given a set of bindings ), return a [keydown handler] that handles them.
...
fn: [RegExpMatchArray]) → [Attrs] | [null] |

[null]

= null,

[joinPredicate]⁠?: fn: [RegExpMatchArray], [node]: [Node]) → [boolean]
...
This is a plugin that adds a type of selection for focusing places that don't allow regular selection .

You'll probably want to load `style/gapcursor.css`, which contains basic styling for the simulated cursor .

By default, gap cursor are only allowed in places where the default content node  is a textblock node. You can customize this by adding an `allowGapCursor` property to your node specs---if it's true, gap cursor are allowed everywhere in that node, if it's `false` they are never allowed.
...
`itemContent` determines the content expression for the list items. If you want the commands defined in this module to apply to your list structure, it should have a shape like `"paragraph block*"` or `"paragraph *"`. `listGroup` can be given to assign a group name to the list node types, for example `"block"`.
